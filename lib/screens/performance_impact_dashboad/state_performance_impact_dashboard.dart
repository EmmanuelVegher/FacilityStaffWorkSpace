...
class AttendanceDataPoint {
  ...
  final String state;
  AttendanceDataPoint(
      {required this.staffId, required this.clockInTime, required this.state});
}
...
class _PunctualityTrendData {
  ...
  final int lateCount;
  _PunctualityTrendData(
      {required this.date, required this.earlyCount, required this.lateCount});
}
...
class StatePerformanceImpactDashboardPage extends StatefulWidget {
  ...
  @override
  _StatePerformanceImpactDashboardPageState createState() =>
      _StatePerformanceImpactDashboardPageState();
}

class _StatePerformanceImpactDashboardPageState
    extends State<StatePerformanceImpactDashboardPage> {
  ...
  // PER-STATE DATA (following StateLevelEngagementReportPage pattern)
  Map<String, int> _callTrackerTotalCallsByState = {},
      _eacTotalCallsByState = {},
      _vlTotalCallsByState = {};
  Map<String, int> _callTrackerTotalDurationByState = {},
      _callTrackerOutgoingAnsweredByState = {},
      _callTrackerMissedByState = {},
      _callTrackerIncomingAnsweredByState = {};
  Map<String, double> _callTrackerOutgoingCostByState = {},
      _callTrackerIncomingCostByState = {};
  Map<String, int> _eacTotalDurationByState = {},
      _eacOutgoingAnsweredByState = {},
      _eacMissedByState = {},
      _eacIncomingAnsweredByState = {};
  Map<String, double> _eacOutgoingCostByState = {},
      _eacIncomingCostByState = {};
  Map<String, int> _vlTotalDurationByState = {},
      _vlOutgoingAnsweredByState = {},
      _vlMissedByState = {},
      _vlIncomingAnsweredByState = {};
  Map<String, double> _vlOutgoingCostByState = {}, _vlIncomingCostByState = {};

  // Global aggregates for current state
  int _callTrackerTotalCalls = 0, _eacTotalCalls = 0, _vlTotalCalls = 0;
  int _callTrackerTotalDuration = 0,
      _callTrackerOutgoingAnswered = 0,
      _callTrackerMissed = 0,
      _callTrackerIncomingAnswered = 0;
  double _callTrackerOutgoingCost = 0.0, _callTrackerIncomingCost = 0.0;
  int _eacTotalDuration = 0,
      _eacOutgoingAnswered = 0,
      _eacMissed = 0,
      _eacIncomingAnswered = 0;
  double _eacOutgoingCost = 0.0, _eacIncomingCost = 0.0;
  int _vlTotalDuration = 0,
      _vlOutgoingAnswered = 0,
      _vlMissed = 0,
      _vlIncomingAnswered = 0;
  ...
  void _clearData() {
    setState(() {
      ...
      _recentlyInactiveStaff = [];
      _moduleActivityData = [];
      _activeFacilitiesByModule = {};
      _totalFilteredFacilities = 0;

      // Clear per-state data Maps
      _callTrackerTotalCallsByState = {};
      _eacTotalCallsByState = {};
      _vlTotalCallsByState = {};
      _callTrackerTotalDurationByState = {};
      _callTrackerOutgoingAnsweredByState = {};
      _callTrackerMissedByState = {};
      _callTrackerIncomingAnsweredByState = {};
      _callTrackerOutgoingCostByState = {};
      _callTrackerIncomingCostByState = {};
      _eacTotalDurationByState = {};
      _eacOutgoingAnsweredByState = {};
      _eacMissedByState = {};
      _eacIncomingAnsweredByState = {};
      _eacOutgoingCostByState = {};
      _eacIncomingCostByState = {};
      _vlTotalDurationByState = {};
      _vlOutgoingAnsweredByState = {};
      _vlMissedByState = {};
      _vlIncomingAnsweredByState = {};
      _vlOutgoingCostByState = {};
      _vlIncomingCostByState = {};

      // Clear global aggregates
      _callTrackerTotalCalls = 0;
      _callTrackerTotalDuration = 0;
      _callTrackerOutgoingAnswered = 0;
      _callTrackerMissed = 0;
      _callTrackerIncomingAnswered = 0;
      _callTrackerOutgoingCost = 0.0;
      _callTrackerIncomingCost = 0.0;
      _eacTotalCalls = 0;
      _eacTotalDuration = 0;
      _eacOutgoingAnswered = 0;
      _eacMissed = 0;
      _eacIncomingAnswered = 0;
      _eacOutgoingCost = 0.0;
      _eacIncomingCost = 0.0;
      _vlTotalCalls = 0;
      _vlTotalDuration = 0;
      _vlOutgoingAnswered = 0;
      _vlMissed = 0;
    ...
    try {
      ...
      final futures = [
        ...
        _firestore
            ...
            .where('trackerState', isEqualTo: _currentState)
            .where('dateTracked', isGreaterThanOrEqualTo: _startDate)
            .where('dateTracked',
                isLessThanOrEqualTo: _endDate.add(const Duration(days: 1)))
            .get(),
        _firestore
            ...
            .where('trackerState', isEqualTo: _currentState)
            .where('callDateTime', isGreaterThanOrEqualTo: _startDate)
            .where('callDateTime',
                isLessThanOrEqualTo: _endDate.add(const Duration(days: 1)))
            .get(),
      ];
  ...
  void _processAllData(
    ...
    _totalFilteredFacilities = filteredStaffMap.values
        .where((s) => s.category == 'Facility Staff' && s.location.isNotEmpty)
        .map((s) => s.location)
        .toSet()
        .length;

    final staffIdToStateMap = {
    ...
    _processModuleActivityData(
        callLogs.docs, eacLogs.docs, vlLogs.docs, staffIdToStateMap);

    // With the corrected query fetching all records/leaves, these sets are now populated correctly from a global context.
    final allStaffWithAttendance =
        allRecords.docs.map((doc) => doc.reference.parent.parent!.id).toSet();
    final allStaffWithLeave =
        allLeave.docs.map((doc) => doc.reference.parent.parent!.id).toSet();

    // This set contains only facility staff from the CURRENT state, as it's derived from filteredStaffMap.
    final facilityStaffIds = filteredStaffMap.values
        .where((s) => s.category == 'Facility Staff')
        .map((s) => s.id)
        .toSet();

    // The logic now works correctly: it finds staff from the current state who are NOT in the global sets of active/on-leave staff.
    final completelyInactive = facilityStaffIds
        .difference(allStaffWithAttendance)
        .difference(allStaffWithLeave);
    _completelyInactiveStaff = completelyInactive
        .map((id) => filteredStaffMap[id])
        .whereType<StaffDetails>()
        .toList();

    final onLeaveNoAttendance =
        allStaffWithLeave.difference(allStaffWithAttendance);
    final inactiveOnLeave = facilityStaffIds.intersection(onLeaveNoAttendance);
    _inactiveStaffOnLeave = inactiveOnLeave
        .map((id) => filteredStaffMap[id])
        .whereType<StaffDetails>()
        .toList();
    ...
    // This list is now populated correctly because `allRecords` contains data.
    // The `if (staffDetails != null)` check effectively filters records to only include staff from the current state.
      ...
      if (staffDetails != null) {
        ...
        if (data['timestamp'] is Timestamp) {
          ...
          if (timestamp.isAfter(
            ...
            dateFilteredAttendance.add(AttendanceDataPoint(
                staffId: staffId,
  ...
  void _calculateRecentlyInactiveStaff(
      QuerySnapshot<Map<String, dynamic>> allRecords,
      Map<String, StaffDetails> filteredStaffMap) {
    ...
    for (final doc in allRecords.docs) {
      final staffId = doc.reference.parent.parent!.id;
      // Filter to only consider staff from the current state
    ...
    lastAttendanceMap.forEach((staffId, lastDate) {
      ...
      if (daysSince > 3) {
        final staffDetails = filteredStaffMap[staffId];
        if (staffDetails != null) {
          inactiveList.add(RecentlyInactiveStaff(
    ...
    inactiveList.sort((a, b) =>
        b.daysSinceLastAttendance.compareTo(a.daysSinceLastAttendance));
    _recentlyInactiveStaff = inactiveList;
  }

  // --- PROCESSING LOGIC (Copied & simplified from HQ Report) ---

  void _processCallMetrics(
      List<QueryDocumentSnapshot<Map<String, dynamic>>> docs,
      Map<String, String> staffStateMap) {
    for (var doc in docs) {
      final data = doc.data();
      final state = staffStateMap[data['trackedById']] ??
          data['trackerFacilityState'] as String?;
      if (state == null) continue;

      final duration = data['callDuration'] as int? ?? 0;
      final status =
          (data['callStatus'] as String?)?.toLowerCase().trim() ?? '';

      // Aggregate by state
      _callTrackerTotalCallsByState[state] =
          (_callTrackerTotalCallsByState[state] ?? 0) + 1;
      _callTrackerTotalDurationByState[state] =
          (_callTrackerTotalDurationByState[state] ?? 0) + duration;

      if (status == 'answered') {
        final cost = duration * _costPerSecond;
        _callTrackerOutgoingAnsweredByState[state] =
            (_callTrackerOutgoingAnsweredByState[state] ?? 0) + 1;
        _callTrackerOutgoingCostByState[state] =
            (_callTrackerOutgoingCostByState[state] ?? 0.0) + cost;
      } else if (status == 'incoming answered') {
        final cost = duration * _costPerSecond;
        _callTrackerIncomingAnsweredByState[state] =
            (_callTrackerIncomingAnsweredByState[state] ?? 0) + 1;
        _callTrackerIncomingCostByState[state] =
            (_callTrackerIncomingCostByState[state] ?? 0.0) + cost;
      } else if (status.contains('missed') ||
          status.contains('not answered') ||
          status.contains('failed')) {
        _callTrackerMissedByState[state] =
            (_callTrackerMissedByState[state] ?? 0) + 1;
      }
    }

    // Update global aggregates for current state only
    if (_currentState.isNotEmpty) {
      _callTrackerTotalCalls =
          _callTrackerTotalCallsByState[_currentState] ?? 0;
      _callTrackerTotalDuration =
          _callTrackerTotalDurationByState[_currentState] ?? 0;
      _callTrackerOutgoingAnswered =
          _callTrackerOutgoingAnsweredByState[_currentState] ?? 0;
      _callTrackerIncomingAnswered =
          _callTrackerIncomingAnsweredByState[_currentState] ?? 0;
      _callTrackerMissed = _callTrackerMissedByState[_currentState] ?? 0;
      _callTrackerOutgoingCost =
          _callTrackerOutgoingCostByState[_currentState] ?? 0.0;
      _callTrackerIncomingCost =
          _callTrackerIncomingCostByState[_currentState] ?? 0.0;
    }
  }

  void _processEacMetrics(
      List<QueryDocumentSnapshot<Map<String, dynamic>>> docs,
      Map<String, String> staffStateMap) {
    for (var doc in docs) {
      final data = doc.data();
      final state =
          staffStateMap[data['trackedById']] ?? data['trackerState'] as String?;
      if (state == null) continue;

      final duration = data['callDuration'] as int? ?? 0;
      final status =
          (data['trackingOutcome'] as String?)?.toLowerCase().trim() ?? '';

      // Aggregate by state
      _eacTotalCallsByState[state] = (_eacTotalCallsByState[state] ?? 0) + 1;
      _eacTotalDurationByState[state] =
          (_eacTotalDurationByState[state] ?? 0) + duration;

      if (status == 'answered' || status == 'completed') {
        final cost = duration * _costPerSecond;
        _eacOutgoingAnsweredByState[state] =
            (_eacOutgoingAnsweredByState[state] ?? 0) + 1;
        _eacOutgoingCostByState[state] =
            (_eacOutgoingCostByState[state] ?? 0.0) + cost;
      } else if (status == 'incoming answered') {
        final cost = duration * _costPerSecond;
        _eacIncomingAnsweredByState[state] =
            (_eacIncomingAnsweredByState[state] ?? 0) + 1;
        _eacIncomingCostByState[state] =
            (_eacIncomingCostByState[state] ?? 0.0) + cost;
      } else if (status.contains('missed') ||
          status.contains('not answered') ||
          status.contains('failed')) {
    ...
    // Update global aggregates for current state only
    if (_currentState.isNotEmpty) {
      _eacTotalCalls = _eacTotalCallsByState[_currentState] ?? 0;
      _eacTotalDuration = _eacTotalDurationByState[_currentState] ?? 0;
      _eacOutgoingAnswered = _eacOutgoingAnsweredByState[_currentState] ?? 0;
      _eacIncomingAnswered = _eacIncomingAnsweredByState[_currentState] ?? 0;
      _eacMissed = _eacMissedByState[_currentState] ?? 0;
      _eacOutgoingCost = _eacOutgoingCostByState[_currentState] ?? 0.0;
      _eacIncomingCost = _eacIncomingCostByState[_currentState] ?? 0.0;
    }
  }

  void _processVlMetrics(List<QueryDocumentSnapshot<Map<String, dynamic>>> docs,
      Map<String, String> staffStateMap) {
    for (var doc in docs) {
      final data = doc.data();
      final state =
      ...
      _vlTotalDurationByState[state] =
          (_vlTotalDurationByState[state] ?? 0) + duration;

      if (status == 'answered' || status == 'completed') {
        final cost = duration * _costPerSecond;
        _vlOutgoingAnsweredByState[state] =
            (_vlOutgoingAnsweredByState[state] ?? 0) + 1;
        _vlOutgoingCostByState[state] =
            (_vlOutgoingCostByState[state] ?? 0.0) + cost;
      } else if (status == 'incoming answered') {
        final cost = duration * _costPerSecond;
        _vlIncomingAnsweredByState[state] =
            (_vlIncomingAnsweredByState[state] ?? 0) + 1;
        _vlIncomingCostByState[state] =
            (_vlIncomingCostByState[state] ?? 0.0) + cost;
      } else if (status.contains('missed') ||
          status.contains('not answered') ||
          status.contains('failed')) {
    ...
    // Update global aggregates for current state only
    if (_currentState.isNotEmpty) {
      _vlTotalCalls = _vlTotalCallsByState[_currentState] ?? 0;
      _vlTotalDuration = _vlTotalDurationByState[_currentState] ?? 0;
      _vlOutgoingAnswered = _vlOutgoingAnsweredByState[_currentState] ?? 0;
      _vlIncomingAnswered = _vlIncomingAnsweredByState[_currentState] ?? 0;
      _vlMissed = _vlMissedByState[_currentState] ?? 0;
      _vlOutgoingCost = _vlOutgoingCostByState[_currentState] ?? 0.0;
      _vlIncomingCost = _vlIncomingCostByState[_currentState] ?? 0.0;
    }
  }

  void _processModuleActivityData(
      List<QueryDocumentSnapshot<Map<String, dynamic>>> callDocs,
      List<QueryDocumentSnapshot<Map<String, dynamic>>> eacDocs,
      List<QueryDocumentSnapshot<Map<String, dynamic>>> vlDocs,
      Map<String, String> staffStateMap) {
    final Map<String, Set<String>> callFacsByState = {},
        eacFacsByState = {},
        vlFacsByState = {};
    ...
    if (_currentState.isNotEmpty) {
      final stateCallFacs = callFacsByState[_currentState] ?? {};
      final stateEacFacs = eacFacsByState[_currentState] ?? {};
      final stateVlFacs = vlFacsByState[_currentState] ?? {};
  ...
  void _processChartData(List<AttendanceDataPoint> data) {
    // The incoming `data` list is already pre-filtered for the current state.
    if (data.isEmpty) {
      // Ensure charts are cleared if no data is found after a reload
      setState(() {
        _adoptionData = [];
        _activeStaffData = [];
        _punctualityTrend = [];
    ...
    for (var record in data) {
      ...
      final eightAm = DateTime(record.clockInTime.year,
    ...
    _punctualityTrend = dailyPunctuality.entries.map((entry) {
      return _PunctualityTrendData(
          date: dayFormat.parse(entry.key),
          earlyCount: entry.value.early,
          lateCount: entry.value.late);
    }).toList()
    ...
    _activeStaffData = monthlyActiveStaff.entries
        .map((entry) => _ChartData(entry.key, entry.value.length.toDouble()))
        .toList()
      ..sort((a, b) => monthFormat
          .parse(a.category)
          .compareTo(monthFormat.parse(b.category)));
    ...
    firstRecordPerStaff.forEach((staffId, firstDate) {
      ...
      monthlyAdoptionCounts[monthKey] =
          (monthlyAdoptionCounts[monthKey] ?? 0) + 1;
    });
    _adoptionData = monthlyAdoptionCounts.entries
  ...
  Future<Map<String, StaffDetails>> _getFilteredStaffDetails() async {
    ...
    // Firestore 'whereIn' queries are limited to 30 items. If more facilities can be selected, this needs chunking.
    // Assuming a reasonable number of facilities for a single state dashboard, this is acceptable.
  ...
  Widget build(BuildContext context) {
    bool hasData = _adoptionData.isNotEmpty ||
        _activeStaffData.isNotEmpty ||
        _punctualityTrend.isNotEmpty ||
        _recentlyInactiveStaff.isNotEmpty ||
        _completelyInactiveStaff.isNotEmpty ||
        _moduleActivityData.isNotEmpty;
  ...
  Widget _buildDashboardBody() {
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          _buildChartCard(
              title: "New User Adoption in $_currentState",
              subtitle: "Staff who recorded their first attendance each month.",
              chart: _buildAdoptionBarChart(_adoptionData),
              isWide: true),
          const SizedBox(height: 24),
          _buildChartCard(
              title: "Monthly Active Staff Trend in $_currentState",
              subtitle: "Unique staff with at least one record each month.",
              chart: _buildActiveStaffLineChart(_activeStaffData),
              isWide: true),
          const SizedBox(height: 24),
          _buildChartCard(
              title: "Daily Clock-in Punctuality Trend in $_currentState",
              subtitle: "Daily count of early vs. late clock-in records.",
              chart: _buildPunctualityTrendChart(_punctualityTrend),
              isWide: true),
          const SizedBox(height: 24),
          ...
          const SizedBox(height: 24),
          _buildModuleActivitySection(_moduleActivityData,
              _activeFacilitiesByModule, _totalFilteredFacilities),
          const SizedBox(height: 24),
  ...
  Widget _buildRecentlyInactiveStaffSection(List<RecentlyInactiveStaff> data) {
    ...
    return _ScrollablePaginatedTable(
      header: Row(children: [
        ...
        Expanded(
            child: Text("Staffs whose last clock-in > 3 days (${data.length})",
                style: TextStyle(
  ...
  Widget _buildCompletelyInactiveStaffSection(List<StaffDetails> data) {
    ...
    return _ScrollablePaginatedTable(
      header: Row(children: [
        ...
        const SizedBox(width: 8),
        Expanded(
            child: Text("Completely Inactive 'Facility Staff' (${data.length})",
                style: TextStyle(
  ...
  Widget _buildInactiveStaffOnLeaveSection(List<StaffDetails> data) {
    ...
    return _ScrollablePaginatedTable(
      header: Row(children: [
        ...
        const SizedBox(width: 8),
        Expanded(
            child: Text("Staff on leave with no attendance (${data.length})",
                style: TextStyle(
  ...
  Widget _buildModuleActivitySection(List<_ChartData> moduleData,
      Map<String, List<String>> facilityData, int totalFacilities) {
    ...
    return Card(
      ...
      child: ExpansionTile(
        key: PageStorageKey<String>(
            'moduleActivityExpansion_${moduleData.hashCode}'),
        initiallyExpanded: true,
        backgroundColor: Colors.blue.shade50,
        collapsedBackgroundColor: Colors.blue.shade50,
        leading:
            Icon(Icons.phone_in_talk_outlined, color: Colors.blue.shade800),
        title: Text("Call tracking Module",
            style: TextStyle(
                fontWeight: FontWeight.bold, color: Colors.blue.shade900)),
        subtitle: const Text(
            "Number of unique facilities using each communication module."),
        children: [
          Padding(
            ...
            child: Wrap(
                ...
                children: [
                  SizedBox(
                      width: 400,
                      height: 350,
                      child: _buildModuleActivityChart(
                          moduleData, totalFacilities)),
                  SizedBox(
                      width: 400,
                      height: 350,
                      child: _buildActiveFacilitiesTable(facilityData))
                ]),
          ),
        ],
      ),
  ...
  Widget _buildMetricSummarySection() {
    return Column(
      ...
      children: [
        Text("Call Tracker summary",
            style: Theme.of(context)
                .textTheme
                .headlineSmall
                ?.copyWith(color: const Color(0xFF722F37))),
        const SizedBox(height: 4),
        Text(
            "Aggregate call metrics for $_currentState within the selected date range.",
        ...
        Wrap(
          ...
          children: [
            _buildModuleSummaryCard(
                ...
                totalCalls: _callTrackerTotalCalls,
                totalDuration: _callTrackerTotalDuration,
                outgoingAnswered: _callTrackerOutgoingAnswered,
                missed: _callTrackerMissed,
                incomingAnswered: _callTrackerIncomingAnswered,
                outgoingCost: _callTrackerOutgoingCost,
                incomingCost: _callTrackerIncomingCost),
            _buildModuleSummaryCard(
                title: "EAC Tracker",
                icon: Icons.support_agent,
                totalCalls: _eacTotalCalls,
                totalDuration: _eacTotalDuration,
                outgoingAnswered: _eacOutgoingAnswered,
                missed: _eacMissed,
                incomingAnswered: _eacIncomingAnswered,
                outgoingCost: _eacOutgoingCost,
                incomingCost: _eacIncomingCost),
            _buildModuleSummaryCard(
                title: "Viral Load Tracker",
                icon: Icons.science,
                totalCalls: _vlTotalCalls,
                totalDuration: _vlTotalDuration,
                outgoingAnswered: _vlOutgoingAnswered,
                missed: _vlMissed,
                incomingAnswered: _vlIncomingAnswered,
                outgoingCost: _vlOutgoingCost,
                incomingCost: _vlIncomingCost),
          ],
        ),
      ],
    );
  ...
  Widget _buildModuleSummaryCard({
    ...
    required int outgoingAnswered,
    required int missed,
    required int incomingAnswered,
    required double outgoingCost,
        ...
        child: Column(
          children: [
            Container(
              ...
              child: Row(children: [
                Icon(icon, color: const Color(0xFF722F37)),
            ...
            Padding(
              ...
              child: Wrap(
                ...
                children: [
                  ...
                  _buildMetricTile(
                      "Total Call Duration", formatDuration(totalDuration)),
                  _buildMetricTile(
                      "Outgoing Answered", outgoingAnswered.toString(),
                      color: Colors.green.shade800),
                  _buildMetricTile(
                      "Incoming Answered", incomingAnswered.toString(),
                      color: Colors.blue.shade800),
                  _buildMetricTile("Missed/Failed", missed.toString(),
                      color: Colors.red.shade800),
                  _buildMetricTile("Est. Outgoing Cost",
                      currencyFormatter.format(outgoingCost),
                      color: Colors.green.shade900),
                  _buildMetricTile("Est. Incoming Cost",
                      currencyFormatter.format(incomingCost),
  ...
  Widget _buildModuleActivityChart(List<_ChartData> data, int totalFacilities) {
    return SfCartesianChart(
        ...
        primaryYAxis: NumericAxis(
            title: const AxisTitle(text: 'Number of Active Facilities'),
              ...
              child: TabBarView(
                children: tabs.map((name) {
                  ...
                  if (facilities.isEmpty) {
                    return const Center(
                        child: Text("No facilities were active."));
                  }
  ...
  Widget _buildAdoptionBarChart(List<_ChartData> data) {
    return SfCartesianChart(
        ...
        primaryYAxis: NumericAxis(
            title: const AxisTitle(text: 'Number of New Staff'),
            numberFormat: NumberFormat.compact()),
  ...
  Widget _buildActiveStaffLineChart(List<_ChartData> data) {
    return SfCartesianChart(
        primaryXAxis: const CategoryAxis(
            labelIntersectAction: AxisLabelIntersectAction.rotate45),
        primaryYAxis: NumericAxis(
            title: const AxisTitle(text: 'Number of Active Staff'),
            numberFormat: NumberFormat.compact()),
  ...
  Widget _buildPunctualityTrendChart(List<_PunctualityTrendData> data) {
    return SfCartesianChart(
      ...
      primaryYAxis: NumericAxis(
          title: const AxisTitle(text: 'Number of Clock-ins'),
          numberFormat: NumberFormat.compact()),
      ...
      series: <CartesianSeries>[
        LineSeries<_PunctualityTrendData, DateTime>(
            ...
            name: 'Early Clock-ins (<= 8:00 AM)',
            color: Colors.green,
            markerSettings: const MarkerSettings(isVisible: true),
            dataLabelSettings: DataLabelSettings(
                isVisible: true,
                builder:
                    (dynamic d, dynamic point, dynamic series, int pI, int sI) {
                  final cd = data[pI];
                  final t = cd.earlyCount + cd.lateCount;
                  if (t == 0) return const Text('');
                  final p = (cd.earlyCount / t) * 100;
...
class _ScrollablePaginatedTable extends StatefulWidget {
  ...
  __ScrollablePaginatedTableState createState() =>
      __ScrollablePaginatedTableState();
}
