...
// FACILITY-LEVEL (MULTI-SELECT) EAC REPORTS PAGE - REWRITTEN WITH UNMASKING FEATURE
import 'dart:async';
...
class _StateEacReportsPageWebState extends State<StateEacReportsPageWeb> {
  ...
  Future<void> _loadCurrentUserBio() async {
    try {
      ...
      if(docSnapshot.exists && mounted) {
        setState(() => _currentUserState = docSnapshot.data()?['state'] as String?);
            ...
            child: Padding(
              ...
              child: DataTable(
                columns: const [
                  ...
                  DataColumn(label: Text('EAC Session')),
  ...
  Future<void> _exportToCSV() async {
    ...
    try {
      ...
      _triggerDownload(utf8.encode(csvData), 'facility_eac_report_${DateFormat('yyyyMMdd').format(DateTime.now())}.csv', 'text/csv');
    } finally {
  ...
  Widget _buildStatusCell(String? status) { if (status == null || status.isEmpty) { return const Text('N/A'); } final lowerStatus = status.toLowerCase(); Color color = _getStatusColor(status); Widget? icon; if (lowerStatus.contains('answered')) { icon = Icon(Icons.call_received, color: color, size: 16); } else if (lowerStatus.contains('failed') || lowerStatus.contains('missed')) { icon = Icon(Icons.phone_missed, color: color, size: 16); } else if (lowerStatus.contains('busy')) { icon = Icon(Icons.phone_in_talk, color: color, size: 16); } if (icon != null) { return Row(mainAxisSize: MainAxisSize.min, children: [ icon, const SizedBox(width: 6), Flexible(child: Text(status, style: TextStyle(color: color, fontWeight: FontWeight.w500))), ],); } return Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: color.withOpacity(0.2), borderRadius: BorderRadius.circular(4)), child: Text(status, style: TextStyle(color: color, fontWeight: FontWeight.w500)),); }
