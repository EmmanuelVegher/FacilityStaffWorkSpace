...
// STATE-WIDE EAC & CALLS TRACKER REPORTS PAGE (FINAL, ROBUST VERSION)
import 'dart:async';
...
class _HqEacReportsPageWebState extends State<HqEacReportsPageWeb> {
  ...
  Future<void> _fetchCallLogs() async {
    final List<String> statesToQuery = _selectedStates.contains('All States')
        ? [] // Empty list means no state filter, i.e., all states
        : _selectedStates;
    ...
    if (statesToQuery.isNotEmpty && facilitiesToQuery.isNotEmpty) {
      _showSnackBar("Fetching logs state-by-state for selected facilities...");
      for (final state in statesToQuery) {
        futures.add(
            baseQuery
                .where('trackerState', isEqualTo: state)
                .where('trackerFacilityLocation', whereIn: facilitiesToQuery)
                .get()
        );
      }
    }
      ...
      if (statesToQuery.isNotEmpty) {
        singleQuery = singleQuery.where('trackerState', whereIn: statesToQuery);
      }
      if (facilitiesToQuery.isNotEmpty) {
        singleQuery = singleQuery.where('trackerFacilityLocation', whereIn: facilitiesToQuery);
      }
  ...
  Future<void> _fetchEacSummaries() async {
    ...
    if (_selectedStates.contains('All States')) {
      _summaryErrorMessage = "National-level summary analysis is not supported. Please select a specific state.";
  ...
  Widget _buildEacAnalysisSection() {
    ...
    String subtitle = 'For ${_selectedFacilities.contains("All Facilities") ? "All Facilities" : "${_selectedFacilities.length} Facilitie(s)"} in ${_selectedStates.length == 1 ? _selectedStates.first : "${_selectedStates.length} States"}';
  ...
  Widget _buildSummaryInfoCard() {
    ...
    if(_selectedStates.contains("All States")){
      facilityCount = 0;
    } else if (_selectedFacilities.contains('All Facilities')) {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Wrap(
          ...
          children: [
            if(!_selectedStates.contains("All States")) _buildInfoTile(iconWidget: Icon(Icons.location_city, color: Colors.blueGrey, size: 36), label: 'Selected Facilities', value: facilityCount.toString()),
  ...
  // --- THIS IS THE CORRECTED AND FINAL VERSION OF THE METHOD ---
  ...
  Future<void> _exportToCSV() async {
    ...
    try {
      ...
      _triggerDownload(utf8.encode(csvData), 'state_eac_report.csv', 'text/csv');
    } finally {
